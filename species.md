# Species List

> [!NOTE]\
> <https://diyeverywhere.com/2023/02/01/the-ultimate-guide-to-when-to-plant-vegetables-and-why/?src=fbfan_74378&t=fbsub_gardenstory&fbclid=IwAR0gYe9MA4VVLw4yTuofhjtf5md9i_exoN0SAQ6kiJgiVCZPT0A8rD_DyIk>

Here is a list of species that we are considering for the garden.  This list is not exhaustive and will be updated as we find more species that we want to include in the garden.

We will organize the species by zone.  We will also include the following information for each species:

- Size
  - Small
  - Medium
  - Large
- Growth Rate
  - Fast
  - Medium
  - Slow
- Shade
  - Full Sun
  - Partial Shade
  - Full Shade
- Biomass
- Mulch
- Edible Parts
- Pollinators
- Nitrogen Fixers
- Soil Builders
- Root Depth
- Planting Season
  - Spring
  - Summer
  - Autumn
  - Winter
- Hardiness Zone

## Table of Contents

- [Species List](#species-list)
  - [Table of Contents](#table-of-contents)
  - [Zone 1](#zone-1)
  - [Zone 2](#zone-2)
  - [Zone 3](#zone-3)
  - [Zone 4](#zone-4)
  - [Zone 5](#zone-5)
  - [List of Plants](#list-of-plants)
    - [Perennials](#perennials)
    - [Ground Covers](#ground-covers)
    - [Shade Tolerant Edibles](#shade-tolerant-edibles)
    - [Food Forest Starter Bundle](#food-forest-starter-bundle)
    - [Vines/Climbers](#vinesclimbers)
    - [Trees](#trees)
      - [Fruiting](#fruiting)
      - [Flowering / Ornamental](#flowering--ornamental)
    - [Vegetables](#vegetables)
    - [Ornamental's and Flowers](#ornamentals-and-flowers)
    - [Drought Tolerant](#drought-tolerant)
    - [Shrubs](#shrubs)
    - [Architectural Foliage](#architectural-foliage)
      - [Japanese Inspired](#japanese-inspired)
    - [Mushrooms](#mushrooms)
      - [Wood](#wood)
      - [Compost](#compost)
      - [Garden](#garden)
      - [Odd Balls](#odd-balls)
      - [Wild Harvest](#wild-harvest)

## Zone 1

## Zone 2

## Zone 3

## Zone 4

## Zone 5

## List of Plants

### Perennials

- sweet potato
- Asparagus
- Egyptian Walking Onion
- Fennel
- Citrus
- New Zealand Spinach
- Calendula
- Rosemary
- Passion Fruit
- Mulberry
- Ginger
- Turmeric
- Shampoo Ginger
- Soap Nuts
- Raspberry
- Strawberry
- Lemon Grass
- Feijoa Pineapple Guava
- Lavender
- Mint (chocolate, spear, pepper, lemon, pineapple, apple, grapefruit) CONTAINERS
- lemon balm
- Sage (big leaf, alpina)
- Apples
- Banana
- Perennial Basil
- Society Garlic
- Bay tree
- Globe Artichoke
- Pepino melon

### Ground Covers

- Purple Creeping Thyme **
- Soleirolia soleirolii (replacement to moss for zone 9) **
- Kurapia (replacement to grass for zone 9) **
- Pachysandra terminalis **
- Herniaria glabra **
- Corsican Mint **
- Aubrita Hydrida **
- Phlox Subulata (rockery) **
  - Alpine
  - Tamaongalei
  - Eva
- Creeping Gazania **
- Sedums (rockeries, non-foot traffic areas) **
  - Angelina (paths)
  - Red Dragons Blood
  - Stone Crop
  - creeping (paths)
- Golden Creeping Jenny (shady area) **
- Sweet Woodruff (shady area) **
- Blue carpet Juniper *
- Buffalo Juniper *
- Bar Harbor Juniper *
- Creeping Manzanita (extremely drought tolerant) *
- Dimondia (rockeries) *
- Blue star creeper
- Scotch Moss (shade/rockeries)
- Iris Moss (shade/rockeries)
- sweet potato (edible)
- Purslane (edible)
- Nasturtium (edible)
- Sweet Violet (edible) (shade)
- Alpine strawberries (edible)
- Lawn Chamomile (edible)
- Prostrate Rosemary (edible)
- Red Clover (edible flowers) (nitrogen fixing)
- Oregano (edible)
- Thyme (edible)
- Lobelia
- Forget me nots
- periwinkle *
- huperizene serata
- alyssum (many varieties) *
- cornus canadensis
- Ajuga reptans
- Persicaria
- Creeping Jenny
- chionodoxa
- Geranium endressi
- Gaultheria
- Calluna vulgaris
  - Gold Haze
- Periwinkle
- Dwarf BugleWeed
- Creeping Speedwell
-

### Shade Tolerant Edibles

- Rainbow Chard
- Arugula
- Lettuce
- Spinach
- Tamarillo (Tree tomato)
- Kale
- Radish
- Blueberry
- Celery
- Parsely
- Cilantro
- Rhubarb
- Bush Beans
- Monstera
- Cardomom Ginger
- Chives
- Mushrooms
- Strawberry Guava
- Guava

### Food Forest Starter Bundle

- Lemongrass
- Sweet potato
- Elderberry
- Mulberry
- Mulberry (dwarf ever-bearing variety)
- Cuban oregano
- Tithonia (sunflower)
- Toilet Paper Plant
- Nopal cactus
- Okinawa spinach
- Longevity spinach
- African blue basil
- Katuk
- Rosemary
- Galangal ginger
- Turmeric
- Pigeon pea/gandules
- Moringa
- Southern/Seminole/Cow pea
- Seminole pumpkin
- Everglades tomato
- Papaya
- East African/Ethiopian kale/Amara
- Cranberry hibiscus
- Molokhia/Egyptian spinach
- Hopi Red Dye amaranth
- Bidens alba
- Sorrel/Roselle
- Luffa
- Daikon radish
- Banana
- Feijoa
- Pineapple
- Borage
- Moringa Olifera

### Vines/Climbers

- Choyote/choco (perennial)
- Malabar Spinach (perennial)
- Campsis radicans
  - Madame Galen
- Grapes (perennial)
  - Leon Millot
- Butterfly Pea (perennial)
- Kiwi (perennial)
- Kiwi Berry (perennial)
- Scarlet Runner Bean (perennial)
- Strawberry
  - Mara des Bois
- lonicera periclymenum (semi-perennial honeysuckle)
- lonicera x tellmanianna (deciduous)
- Cobaea Scandens (perennial exotic)
  - cobaea scandens f. alba (white version)
- Virginia Creeper
- Akebia quinata
- Wisteria
  - For pergolas and arches, choose a Japanese wisteria and for walls, opt for Chinese wisteria (w.sinensis is a good one)
- passiflora caerulea
- passiflora incarnata
- solanum crispum (semi-perennial)(exotic)
- clematis
  - early flower: Pamela Jackman
  - mid-season flower: Armandii
  - late-season flower: Etoile Violette
- trachelospermum jasminoides
- fallopia baldschuanica

### Trees

#### Fruiting

- Kumquat
- Feijoa (named variates)
- Apple
- Peach
- Plum
- Cherry
  - Ornamental
    - Prunus yedoensis
    - Pink Shell
    - Prunus incisa 'The Bride'
- Hawaiian Guava
- Cherry Guava
- White mulberry
- Black Mulberry
- Blood Lime
- Kefir Lime
- Bay tree
- Papaya
- Citrus
- Nuts
- Stone fruit
- Pomegranate
- Finger lime
- Fig
- Nectarine
- Olive
- Jujube

#### Flowering / Ornamental

- Silver Birch
  - Betula utilis jacqumontii
  - Snow Queen
- Amelanchier lamarkii 'Ballerina'
  - Snow Flakes
- Magnolia
  - Magnolia x soulangeana
  - Magnolia Grandiflora
- Crab Apple
  - malus ButterBall
  - Wisely Crab
  - Malus x zumi
- Acer
  - Acer palmatum 'Orange Dream'
  - Acer palmatum 'Sango-kaku
  - Acer palmatum 'Osakazuki
  - Acer palmatum 'Butterfly
  - Acer palmatum 'Garnet
- Cornus kousa
  - Cornus kousa 'Angyo Dwarf'
- Mountain Ash (Rowan)
  - Sorbus vilmorinii
- Crataegus persimilis ‘Prunifolia’
- Hybridus Pendulus
  - Juliette
- Photinia x fraseri 'Red Robin'
- Halesia carolina
- Eucryphia Nymansensis
- Cercis chinensis
  - Avondale
- Davidia involucrata
- bay
- holly
- Prunus lusitanica
- Lagerstroemia

### Vegetables

- Thyme
- Oregano
- Chives
- Majoram
- Arugula
- Mizuna
- Spinach
  - Baby
  - Viking
  - Climbing
  - Egyptian
- Tomatoes
- Chillies
- Cucumbers
- Aubergines
- Broccoli
- Cauliflower
- Fava Beans
- Peppers
  - Rezha Macedonian Heirloom
  - Golden Nugget
  - Black Jalapeno
  - Candy Cane
  - Purple Tiger
  - Tobago Seasoning
  - Yellow Scotch Bonnet
  - Yellow Ghost
  - Murasaki Purple
  - Banana Pepper
  - Aji Amarillo
  - Aji Pineapple
  - Aji Lemon Drop
  - Aji Charapita
  - Aji Panca
  - Aji Omnicolor
  - Aji Fantasy

### Ornamental's and Flowers

- Muscari
- Salvia uliginosa
- Verbena bonariensis
- ethereal stipa
- Hydrangea paniculata
- white gaura
- heleniums
- Alliums
- echinacea
- dahlias
  - Akita
  - Reggae
- helianthus
- asters
- salvias
- ornamental cabbages
- rainbow chard
- rosemary
- mint
- chives
- Senecio
  - 'Angel Wings'
- Alchemilla mollis
- chocolate lace flower
- brook flower
- fennel
- Bowls Mauve wallflower
- sweet peas
- Nepeta
  - 'Walkers Low'
- Digitalis
- delphiniums
- penstemons
- agapanthus
- daffodils
- tulips
- gladiolus
- lavender
- geraniums
- ferns
- nepeta
- astrantia
- Rudbeckia
  - 'Enchanted Embers'
- Sunflowers
  - Velvet Queen
- cosmos
- cornflowers
- poppies
- yarrow
- Ammi majus
- hostas
- coleous
- bergenia
- epimedium
- boston ferns
- Lysimachia nummularia
  - 'Aurea'
- campanula
- lobelia
- fuchsia
- pelargonium
- vinca minor
- ajuga
- begonia
  - Million Kisses
- Ornamental Grass
  - Carex oshimensis
    - Evergold (sedge)
- Roses
  - Climbing
    - Gertrude Jekyll
    - Constance Spry
    - Aloha
    - Blairii Number Two
    - Mortimer Sackler
    - The Generous Gardener
    - Claire Austin
    - The Pilgrim
    - Princes Trust
    - James Galway
    - Iceberg
    - Phyllis Bide
    - St. Swithin
    - Arthur Bell
    - Super Fairy
    - Malvern Hills
    - Rambling Rector
    - Kew Rambler
    - Francis E. Lester
    - Paul's Himalayan Musk
    - Bobbie James
    - Shropshire Lass
    - Blush Noisette
- osteospermum
- gazania
- Christia obcordata
- Freesia (very strong perfume)

### Drought Tolerant

- Rosemary
- Lavender
- Thyme
- Bay
- Olive
- Italian Cypress
- Agapanthus
- Santolina
- Caryopteris
  - Clandonensis
- Chusan Palm
- Dierama pulcherrimum
- Bupleurum Fruticosum
- Anthericum liliago
  - Major
- Hylrotelephium telephium
  - Purple Emperor
- Bearded Iris
  - Ballyhoo
- Oenothera Lindheimeri
  - Whirling Butterflies
- Salvia Patens
- Tulbaghia Violacea
- Sempervivum Arachnoideum (rockery)
- Artemesia Ludoviciana
  - Valerie Finnis
- Convolvulus Cneorum
- Cistus (rock rose)
  - Graysword Pink AGM
  - Alan Fradd
  - Decumbens AGM
  - Ladanifer (used in perfume **)
  - Maculatus AGM
  - X Purpureus AGM
  - Snow Fire AGM
  - x Cyprius AGM
  - Silver Pink
  - Jest AGM
  - Sunset AGM
- Celmisia Semicordata
- Erigeron Karvinskianus (rockery)
- HollyHock
- Euphorbia
  - characias
  - Mellifera
  - Wulfenii
  - cyparissias (ground cover) **
- Eryngium
  - Peacock Blue
  - Big Blue
  - Silver Ghost
- Argyrocytisus Battandieri
- Kniphofia (Red Hot Pokers)
  - Mango Popsicle
  - Traffic Lights
- Cercis siliquastrum
- tamarisk
- Hibiscus
- Androsace Studiosorum (rockery)
  - Salmon's Variety
- Erodium (rockery)
  - Matilda's Purple
- Globularia Cordifolia (rockery)
- Lewisia Cotyledon (rockery)
  - Darcies Gems
- Scleranthus Biflorus (rockery) (alternative to moss **)
- Sedum Spathulifium (rockery)
- Sedum kamtshaticum (rockery)
  - Variegatum
- Crocus (rockery)
  - Chrysanthus
  - Snow Bunting
  - Blue Pearl
  - Advanced
- Dianthus (rockery)
  - La Bourboule
  - Evening Star * (others in the Star series)
  - Memories
- Gentiana Verna(rockery)
- Pulsatilla vulgaris (rockery)
  - Alba
  - Pearl Bells
- Allium Sikkimense (rockery)
- Crepis Incana (rockery)

### Shrubs

- Viburnum Tinus
- Rose Suffolk
- Lavender
- Box-leaved / Japanese holly
- Lace-cap hydrangea
- Gaultheria
- Cordyline
- Daphne
  - Eternal Fragrance
- Camellia
  - Anticipation

### Architectural Foliage

- Cordyline
  - Australis
  - Pink Passion
- Echinops
  - Veitch's Blue
  - Arctic Glow
- Ficus Carica
- Musa Basjoo
- Sweet Fennel
- Agave
- Hakonechola Macra
- Fatsia Japonica
- Hosta
  - Halcyon
  - So Sweet
  - Sum and Substance
- Heuchera (rockery)
  - Chocolate (great for paths)
  - Purple Petticoats
  - Pulchellum (dwarf)
- Eryngium Bourgatti
- Stachys byzantina
- Asparagus aethiopicus
  - Asparagus densiflorus
- Tradescantia
  - Zebrina Violet

#### Japanese Inspired

- Japanese Maples
- Bamboo
- Moss
- Flowering Cherry
- Azaleas
- Fatsia japonica (damp shady spots)
- Hakonechloa macra (ornamental grass) (sunny)
- common box (ornamental grass) (sunny)
- colorful pieris (sunny)
- Acer Palmatum
  - Beni-otake
  - Katsura
- Iris Ensata
  - Rose Queen
- Pinus Mugo
  - Mops
- Enkianthus campanulatus
- Rhododendron
  - Snow Hill
- Prunus incisa
  - The Bride
- Wisteria floribunda
  - Domino
- Primula japonica
  - Miller’s Crimson

### Mushrooms

Here is a list of all the ideal mushrooms for growing.

Medicinal and edibles will be marked separately.

#### Wood

- Oyster (edible)
  - Lung
  - King
  - Pink
  - Yellow
- Shiitake (edible)
- Lions Mane (medical)(edible)
- Enokitake (edible)
- Jelly Ear (edible)
- Reishi (medical)
- Turkey Tail (edible)
- Omphalotus N.

#### Compost

- Agaricus (edible)
- Psilocybe (medical)

#### Garden

- Stropharia (edible)
  - Wine Cap
  - King
  - Yellow
- Morel (edible)
  - Black
  - White

#### Odd Balls

- Cordyceps (medical)

#### Wild Harvest
